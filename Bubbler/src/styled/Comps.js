...
export const MessageBubble = styled.View`
	max-width: 80%;
	margin-bottom: ${props => props.self && props.nextSelf || !props.self && !props.nextSelf && props.type != 'system' ? '3px' : '13px'};
	border-radius: 26px;
	align-self: ${props => props.type == 'system' ? 'center' : props.self == true ?  'flex-end' : 'flex-start'};
	padding: 13px 18px;
	background-color: ${props => props.type == 'system' ? props.theme.background.lighter : props.self == true ? props.theme.background.mainHighlight : props.theme.background.light};
`
