...
const PrivateChatScreen = ({ navigation }) => {
	...
	const [participant, setParticipant] = useState(() => privateChat ? selectPrivCListParticipantByPrivCId(id) : false)
	const [title, setTitle] = useState(() => {

		if (!params.title) {
			let particList = [
				...selectPrivCParticList_Admin(privateChat.participantsList),
				...selectPrivCParticList_Active(privateChat.participantsList),
				...selectPrivCParticList_Pending(privateChat.participantsList),
				...selectPrivCParticList_Inactive(privateChat.participantsList),
				...selectPrivCParticList_Flagged(privateChat.participantsList),
				...selectPrivCParticList_Blocked(privateChat.participantsList)]
	...
	useEffect(() => {
	  ...
	  const unsubscribe = navigation.addListener('focus', async () => {
	  	...
	  	if (reSelectedPrivateChat) setTitle(() => {
	  		if (!params.title) {
	  			let particList = [
					...selectPrivCParticList_Admin(privateChat.participantsList),
					...selectPrivCParticList_Active(privateChat.participantsList),
	...
	const loadPrivCMsgList = async privCMsgListId => {
		...
		if (fetchedPrivCMsgList) {
			var test = await dispatch(privCMsgListFetched(fetchedPrivCMsgList))
			return true
		}
		return false
	}
	...
	const renderedMessage = ({ item, index }) => {
		...
		let type = item.type
		console.log('participant',item.participant, item)

		if (type == 'system') {
			item.content = dateToTimeDateLabel(item.createdAt)
		} 
		return(
		  	<MessageBubble type={type} self={self} nextSelf={nextSelf}>
				<BodyText white={self ? true : false}><Span>{!self ? type != 'system' ? `${user.firstName} `: '' : '' }</Span>{item.content}</BodyText>
			</MessageBubble>)
	}
